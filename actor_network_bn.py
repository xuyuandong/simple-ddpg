...
class ActorNetwork:
	...
	def __init__(self,sess,state_dim,action_dim):
		...
		# create eval actor network
		self.state_input,self.action_output,self.net,self.is_training = self.create_eval_network(state_dim,action_dim)
	...
	def create_eval_network(self,state_dim,action_dim):
		...
		state_input = tf.placeholder("float",[None,state_dim])  #TODO
		...
		layer0_bn = self.batch_norm_layer(state_input,training_phase=is_training,scope_bn='eval_batch_norm_0',activation=tf.identity)
		layer1 = tf.matmul(layer0_bn,W1) + b1
		layer1_bn = self.batch_norm_layer(layer1,training_phase=is_training,scope_bn='eval_batch_norm_1',activation=tf.nn.relu)
		layer2 = tf.matmul(layer1_bn,W2) + b2
		layer2_bn = self.batch_norm_layer(layer2,training_phase=is_training,scope_bn='eval_batch_norm_2',activation=tf.nn.relu)
